...
class Mage:
    def __init__(self,name, type, health, max_health, attack):
        ...
        temp_list = []
        for i in range (7):
            img = pygame.image.load(f'Character/{type}/Mage/{self.name}/Heal/{i}.png').convert_alpha()
            img = pygame.transform.scale(img, (img.get_width() * 3, img.get_height() * 3))
            temp_list.append(img)
        self.animation_list.append(temp_list)
        self.image = self.animation_list[self.action][self.frame_index]
...
Player_obj = Mage('Samsudin', 'Player', 100,100, 10)
Enemy_obj = Mage('Banaspati', 'Enemy', 100,100, 10)
